// Icon atlas for Deck.gl IconLayer
// White silhouette icons on transparent background, tinted via mask:true
const ICON_SIZE = 64;
const COLS = 5;
const ROWS = 3;
...
// Maps generator fuel code to icon name
...
// Maps water facility type code to icon name
...
// All datacenter types share the server-rack icon (color carries the type)
export function datacenterIconName() {
  return "datacenter";
}
...
// --- Drawing functions ---
// Each draws a white filled shape centered at (cx, cy) with radius s
...
function drawAtom(ctx, { cx, cy }, s) {
  ctx.fillStyle = "white";
  // 3 overlapping circles in triangle
  const r = s * 0.32;
  const d = s * 0.42;
  for (let i = 0; i < 3; i++) {
...
function drawServerRack(ctx, { cx, cy }, s) {
  ctx.fillStyle = "white";
  // Rack body
  const w = s * 1.3;
  const h = s * 1.7;
  ctx.fillRect(cx - w / 2, cy - h / 2, w, h);
  // Cut three horizontal unit slots with an LED dot left intact
  ctx.globalCompositeOperation = "destination-out";
  for (let i = 0; i < 3; i++) {
    const y = cy - h / 2 + h * (0.18 + i * 0.3);
    ctx.fillRect(cx - w * 0.32, y, w * 0.64, h * 0.13);
  }
  ctx.globalCompositeOperation = "source-over";
  for (let i = 0; i < 3; i++) {
    const y = cy - h / 2 + h * (0.18 + i * 0.3) + h * 0.065;
    ctx.beginPath();
