defmodule Mix.Tasks.PowerModel.AuditData do
  ...
  defp audit_generators do
    ...
    tiny_cap =
      Repo.one(from g in Generator, where: g.p_max_mw > 0.0 and g.p_max_mw < 0.1, select: count())

    huge_cap =
      Repo.all(
        from g in Generator,
          where: g.p_max_mw > 4000.0,
          select: {g.id, g.p_max_mw, g.fuel_type, g.eia_plant_id},
          order_by: [desc: g.p_max_mw],
          limit: 10
      )
    ...
    for {id, cap, fuel, eia} <- huge_cap,
        do: IO.puts("    gen##{id} #{Float.round(cap, 1)}MW fuel=#{fuel} eia=#{eia}")
    ...
    no_eia =
      Repo.one(
        from g in Generator,
          where: is_nil(g.eia_plant_id) or g.eia_plant_id == "",
          select: count()
      )
    ...
    for {fuel, cnt, mw} <- fuels do
      IO.puts(
        "    #{String.pad_trailing(fuel || "NULL", 12)} #{String.pad_leading("#{cnt}", 6)} units  #{String.pad_leading(Float.round(mw / 1000, 1) |> to_string(), 8)} GW"
      )
    end

    total_cap =
      Repo.one(from g in Generator, where: g.status == "in_service", select: sum(g.p_max_mw))

    IO.puts("  Total in-service capacity: #{Float.round(total_cap / 1000, 1)} GW")

    IO.puts(
      "  EIA reality check: ~1,300 GW nameplate → #{if abs(total_cap / 1000 - 1300) < 200, do: "✓ REASONABLE", else: "⚠ DIVERGENT"}"
    )
  ...
  defp audit_transmission_lines do
    ...
    in_service =
      Repo.one(from t in TransmissionLine, where: t.status == "in_service", select: count())

    IO.puts("  Total: #{total} (#{in_service} in service)")

    null_from =
      Repo.one(from t in TransmissionLine, where: is_nil(t.from_bus_id), select: count())

    null_to = Repo.one(from t in TransmissionLine, where: is_nil(t.to_bus_id), select: count())

    self_loop =
      Repo.one(
        from t in TransmissionLine,
    ...
    null_r =
      Repo.one(
        from t in TransmissionLine, where: is_nil(t.r_pu) or t.r_pu == 0.0, select: count()
      )

    null_x =
      Repo.one(
        from t in TransmissionLine, where: is_nil(t.x_pu) or t.x_pu == 0.0, select: count()
      )
    ...
    null_rating =
      Repo.one(from t in TransmissionLine, where: is_nil(t.rating_a_mva), select: count())

    zero_rating =
      Repo.one(from t in TransmissionLine, where: t.rating_a_mva <= 0.0, select: count())
    ...
    voltages =
      Repo.all(
        from t in TransmissionLine,
          ...
          group_by:
            fragment(
              "CASE WHEN voltage_kv >= 345 THEN '345+' WHEN voltage_kv >= 230 THEN '230-344' WHEN voltage_kv >= 115 THEN '115-229' WHEN voltage_kv >= 69 THEN '69-114' ELSE '<69' END"
            ),
          select:
            {fragment(
               "CASE WHEN voltage_kv >= 345 THEN '345+' WHEN voltage_kv >= 230 THEN '230-344' WHEN voltage_kv >= 115 THEN '115-229' WHEN voltage_kv >= 69 THEN '69-114' ELSE '<69' END"
  ...
  defp audit_substations do
    ...
    rutgers =
      Repo.one(from s in Substation, where: like(s.hifld_id, "rutgers_%"), select: count())
    ...
    null_voltage =
      Repo.one(from s in Substation, where: is_nil(s.max_voltage_kv), select: count())

    null_name =
      Repo.one(from s in Substation, where: is_nil(s.name) or s.name == "", select: count())

    unknown_name =
      Repo.one(from s in Substation, where: like(s.name, "UNKNOWN%"), select: count())
  ...
  defp audit_buses do
    ...
    sources =
      Repo.all(
        from b in Bus, group_by: b.source, select: {b.source, count()}, order_by: [desc: count()]
      )
    ...
    intercos =
      Repo.all(
        from b in Bus,
          ...
          select: {b.interconnection_id, count()},
          order_by: b.interconnection_id
      )
    ...
    orphan_count =
      Repo.one(
        from b in Bus,
          left_join: g in PowerModel.Grid.Generator,
          on: g.bus_id == b.id,
          left_join: l in PowerModel.Grid.Load,
          on: l.bus_id == b.id,
    ...
    types =
      Repo.all(
        from b in Bus, group_by: b.bus_type, select: {b.bus_type, count()}, order_by: b.bus_type
      )
  ...
  defp audit_loads do
    ...
    huge =
      Repo.all(
        from l in Load, where: l.p_mw > 2000.0, select: {l.id, l.p_mw, l.bus_id}, limit: 10
      )
    ...
    IO.puts(
      "  EIA US peak demand: ~750 GW → #{if total_mw / 1000 > 300 and total_mw / 1000 < 1500, do: "✓ REASONABLE", else: "⚠ CHECK"}"
    )
  end
  ...
  defp audit_eia_hourly do
    ...
    if load_count > 0 do
      null_demand =
        Repo.one(from h in HourlyLoadProfile, where: is_nil(h.demand_mw), select: count())

      IO.puts("  Null demand values: #{null_demand}")

      neg_demand =
        Repo.one(from h in HourlyLoadProfile, where: h.demand_mw < 0.0, select: count())

      IO.puts("  Negative demand: #{neg_demand}")

      pjm_stats =
        Repo.one(
          from h in HourlyLoadProfile,
      ...
      if pjm_stats do
        ...
        IO.puts(
          "    Reality: PJM peak ~150 GW, trough ~60 GW → #{if pmax < 200_000 and pmin > 30_000, do: "✓ REASONABLE", else: "⚠ CHECK"}"
        )
      end
      ...
      if erco_stats do
        ...
        IO.puts(
          "    Reality: ERCOT peak ~85 GW → #{if emax < 120_000 and emin > 15_000, do: "✓ REASONABLE", else: "⚠ CHECK"}"
        )
      end
      ...
      if us48_stats do
        ...
        IO.puts(
          "    Reality: US peak ~750 GW → #{if umax < 900_000 and umin > 200_000, do: "✓ REASONABLE", else: "⚠ CHECK"}"
        )
      end
    end
  end
  ...
  defp audit_cross_references do
    ...
    ghost_line_from =
      Repo.one(
        from t in PowerModel.Grid.TransmissionLine,
          left_join: b in PowerModel.Grid.Bus,
    ...
    ghost_load =
      Repo.one(
        from l in PowerModel.Grid.Load,
          left_join: b in PowerModel.Grid.Bus,
          on: l.bus_id == b.id,
    ...
    ghost_xfmr =
      Repo.one(
        from t in PowerModel.Grid.Transformer,
          left_join: b1 in PowerModel.Grid.Bus,
          on: t.from_bus_id == b1.id,
          left_join: b2 in PowerModel.Grid.Bus,
          on: t.to_bus_id == b2.id,
          where:
            (not is_nil(t.from_bus_id) and is_nil(b1.id)) or
              (not is_nil(t.to_bus_id) and is_nil(b2.id)),
          select: count()
      )
    ...
    IO.puts(
      "  Gen/Load ratio: #{Float.round(ratio, 2)} (#{Float.round(gen_mw / 1000, 1)} GW / #{Float.round(load_mw / 1000, 1)} GW)"
    )

    IO.puts(
      "    Typical reserve margin: 1.15-1.40 → #{if ratio > 1.0 and ratio < 2.0, do: "✓ REASONABLE", else: "⚠ CHECK"}"
    )
  end
  ...
  defp audit_geographic_consistency do
    ...
    outside_texas =
      Enum.count(ercot_gens, fn coords ->
        case coords do
    ...
    east_of_divide =
      Enum.count(west_gens, fn coords ->
        case coords do
