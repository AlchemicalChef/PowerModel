defmodule PowerModel.Ingestion.HIFLD.TransmissionLines do
  ...
  def backfill_hifld_fields do
    ...
    total = Repo.one(from tl in TransmissionLine,
    ...
    |> Stream.chunk_every(500)
    |> Stream.each(fn batch ->
      updates = Enum.map(batch, fn %{"attributes" => attrs} ->
      ...
      lines = Repo.all(from tl in TransmissionLine,
  ...
  defp insert_batch(batch) do
    ...
    entries = Enum.map(batch, fn attrs ->
      %{
    ...
    Repo.insert_all(TransmissionLine, entries, on_conflict: :nothing, conflict_target: [:source, :source_id])
  end
