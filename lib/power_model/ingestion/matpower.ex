defmodule PowerModel.Ingestion.Matpower do
  ...
  def import(file_path, opts \\ []) do
    ...
    Logger.info(
      "Parsed: #{length(data.buses)} buses, #{length(data.gens)} generators, #{length(data.branches)} branches"
    )
  ...
  defp import_branches_batch(data, bus_map, bus_kv_map, source, case_name) do
    ...
    line_count =
      ...
      |> Enum.reduce(0, fn batch, total ->
        {count, _} =
          Repo.insert_all(TransmissionLine, batch,
            on_conflict: :nothing,
            conflict_target: [:source, :source_id]
          )
  ...
  defp import_substations_batch(data, case_name) do
    ...
    entries =
      ...
      |> Enum.map(fn {sub_name, bus_entries} ->
        voltages =
          Enum.map(bus_entries, fn {_name, idx} ->
    ...
    |> Enum.reduce(0, fn batch, total ->
      {count, _} =
        Repo.insert_all(Substation, batch, on_conflict: :nothing, conflict_target: [:hifld_id])
