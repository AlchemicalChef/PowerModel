defmodule PowerModel.Failure.Contingency do
  ...
  def screen_n1(lines, base_flows, lodf, rating_threshold \\ 1.0) do
    ...
    |> Enum.flat_map(fn {outaged_line, k} ->
      violations = Enum.with_index(lines)
  ...
  defp compute_ptdf(lines, bus_index, b_prime_inv) do
    # PTDF(l, n) = (1/x_l) * (B'^-1(from_l, n) - B'^-1(to_l, n))
    # b_prime_inv is a list of lists (n-1 x n-1 matrix, slack row/col removed)
    # bus_index maps bus_id -> index in the reduced system (0-based, slack excluded)
    ...
    Enum.map(lines, fn line ->
      ...
      for n <- 0..(n_reduced - 1) do
        inv_from = if from_idx != nil and from_idx < n_reduced do
          Enum.at(Enum.at(b_prime_inv, from_idx, []), n, 0.0)
        else
          0.0
        end

        inv_to = if to_idx != nil and to_idx < n_reduced do
