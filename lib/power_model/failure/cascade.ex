defmodule PowerModel.Failure.Cascade do
  ...
  defp compute_base_overloads(snapshot, dispatch, base_mva) do
    ...
    try do
      ...
      categories = solution.line_flows
  ...
  defp apply_headroom(state, deficit_mw, gens) do
    headrooms =
      Enum.map(gens, fn g ->
        ...
        {g.id, max(g.p_max_mw - current, 0.0)}
      end)
    ...
    if total_headroom <= 0.0 do
      ...
      new_dispatch =
        Enum.reduce(headrooms, state.dispatch, fn {gen_id, headroom}, d ->
          increase = headroom * fraction
          current = Map.get(d, gen_id, 0.0)
  ...
  defp do_cascade(state, step_results, callback) do
    ...
    {water_trips, newly_affected} =
      check_facility_power_loss(
        state.water_facilities, state.affected_water_facilities, dead_bus_ids,
        "water_facility", &water_facility_details/1
      )

    {datacenter_trips, newly_affected_dcs} =
      check_facility_power_loss(
        state.datacenters, state.affected_datacenters, dead_bus_ids,
        "datacenter", &datacenter_details/1
      )

    facility_trips = water_trips ++ datacenter_trips
    ...
    if Enum.empty?(non_thermal_trips) and Enum.empty?(timed_overloads) do
      ...
      # For thermal/zone3 overloads: trip ONLY the fastest (shortest trip time)
  ...
  defp solve_islands_timed(islands, buses, lines, transformers, generators, loads, base_mva, base_overloaded \\ MapSet.new()) do
  ...
  defp compute_timed_overloads(line_flows, base_overloaded \\ MapSet.new()) do
    line_flows
  ...
  # Pick the single component with the shortest trip time.
  ...
  # the solver's P_inject = dispatch_mw / base_mva.
  ...
  # All bus IDs in dead islands (no active generation) — everything on these
  # buses is blacked out.
  defp dead_island_buses(islands, active_gens) do
    ...
    Enum.reduce(islands, MapSet.new(), fn island, acc ->
      if Enum.any?(island, &MapSet.member?(gen_bus_ids, &1)) do
        acc
      else
  ...
  defp check_facility_power_loss(facilities, already_affected, dead_bus_ids, component_type, details_fn) do
    facilities
    |> Enum.filter(fn f ->
      f.bus_id != nil and
  ...
  defp datacenter_details(dc) do
    %{name: dc.name, facility_type: dc.facility_type, operator: dc.operator, power_mw: dc.power_mw}
  end
end
