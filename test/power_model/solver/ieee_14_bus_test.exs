defmodule PowerModel.Solver.IEEE14BusTest do
  ...
  @buses [
    ...
    # Bus 9 carries the case's 19 MVAr shunt capacitor bank
  ...
  describe "DC power flow on IEEE 14-bus" do
    ...
    test "DC totals are populated (lossless identities)", %{solution: sol} do
      total_load = Enum.sum(Enum.map(@loads, & &1.p_mw))

      assert_in_delta sol.total_load_mw, total_load, 0.01
      assert_in_delta sol.total_gen_mw, sol.total_load_mw, 1.0e-6
      assert sol.total_loss_mw == 0.0
      assert is_number(sol.scheduled_gen_mw)
      assert is_number(sol.slack_injection_mw)
      assert is_number(sol.mismatch_mw)
      assert Solution.energy_balance(sol).ok
    end

    test "DC line flows carry rating_mva", %{solution: sol} do
      for {_key, flow} <- sol.line_flows do
        assert Map.has_key?(flow, :rating_mva)
      end
    end
  end
  ...
  describe "AC power flow on IEEE 14-bus" do
    ...
    test "AC voltage magnitudes within 0.5% of published IEEE values", %{solution: sol} do
      # PV buses hold their scheduled setpoints exactly; PQ voltages are solved
      # for. With the bus-9 shunt capacitor modeled, the solution matches the
      # published case to well under 0.1% — 0.5% leaves headroom without
      # letting a missing-shunt-class regression through.
      for {bus_id, expected_vm} <- @expected_ac_vm do
        actual_vm = vm_for(sol, bus_id)
        pct_error = abs(actual_vm - expected_vm) / expected_vm * 100.0

        assert pct_error < 0.5,
               "Vm at bus #{bus_id}: expected ~#{expected_vm} pu, " <>
                 "got #{Float.round(actual_vm, 4)} pu (#{Float.round(pct_error, 2)}% error)"
      end
    end

    test "PQ bus voltages within 0.5% of published IEEE values", %{solution: sol} do
      pq_bus_ids =
        @buses
    ...
    test "line 1-2 flow matches the published solution", %{solution: sol} do
      # Published IEEE 14-bus: 156.88 MW leaves bus 1 toward bus 2
      flow = Solution.line_flow(sol, :line, 1)
      assert_in_delta flow.p_flow_mw, 156.88, 2.0
    end
    ...
    test "real losses match the published solution and the energy balance closes", %{
      solution: sol
    } do
      # Published IEEE 14-bus total real losses: 13.393 MW
      assert_in_delta sol.total_loss_mw, 13.393, 0.2

      assert_in_delta sol.total_gen_mw, sol.total_load_mw + sol.total_loss_mw, 0.5
      assert Solution.energy_balance(sol).ok
    end

    test "slack injection and mismatch are populated when converged", %{solution: sol} do
      assert is_number(sol.slack_injection_mw)
      assert sol.slack_injection_mw != 0.0
      assert is_number(sol.mismatch_mw)
      assert sol.slack_bus_id != nil
    end

    test "AC line flows carry rating_mva", %{solution: sol} do
      for {_key, flow} <- sol.line_flows do
        assert Map.has_key?(flow, :rating_mva)
      end
    end
  ...
  # ── AC Power Flow with voltage-dependent (ZIP) loads ─────────────────

  describe "AC power flow with ZIP loads on IEEE 14-bus" do
    @describetag :slow

    test "converges quadratically with residential (voltage-dependent) loads" do
      # ZIP load derivatives feed the J2/J4 Jacobian diagonals; without them
      # convergence near the solution degrades to linear and iteration counts
      # climb. Residential has the strongest voltage dependence (z=0.4, i=0.3).
      zip_loads = Enum.map(@loads, &Map.put(&1, :load_type, "residential"))
      snapshot = %{build_snapshot() | loads: zip_loads}

      {:ok, sol} = NewtonRaphson.solve(snapshot, solver_opts())
