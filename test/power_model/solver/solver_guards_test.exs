defmodule PowerModel.Solver.SolverGuardsTest do
  @moduledoc """
  SOL-4 / SOL-10 / SOL-11 regression tests.

  The pure-Elixir Gaussian fallback is capped so it can never grind for
  hours inside a GenServer call; the Newton-Raphson Gaussian fallback errors
  on a singular Jacobian instead of fabricating a step; and both solvers
  reject snapshots with duplicate bus ids instead of silently corrupting
  matrix sizing.
  """
