defmodule PowerModel.Transient.ClassicalTest do
  ...
  defp smib_state(p_mech \\ 0.8) do
    ...
    %State{
      ...
      gen_ids: [1, 2],
      gen_bus_ids: [1, 2],
      # Gen at delta_0, infinite bus at 0
      delta: [delta_0, 0.0],
      # Both at synchronous speed
      omega: [1.0, 1.0],
      # Infinite bus absorbs what gen produces
      p_mech: [p_mech, -p_mech],
      # Gen E'=1.2, infinite bus V=1.0
      e_prime: [1.2, 1.0],
      # Large H for infinite bus
      h: [5.0, 10000.0],
      # No damping for clean test
      d: [0.0, 0.0],
      y_red_rows: [0, 0, 1, 1],
      y_red_cols: [0, 1, 0, 1],
  ...
  describe "SMIB steady state" do
    test "generator at steady-state operating point remains stable" do
      state = smib_state()
      # 1 second at 1ms
      trajectory = Classical.simulate(state, 1000, 100)

      # Frequency should stay at 60 Hz (omega = 1.0)
      last = List.last(trajectory)
      [gen_omega, _inf_omega] = last.omega

      assert_in_delta gen_omega, 1.0, 0.001, "Generator should stay at synchronous speed"
    end
    ...
    test "P_elec matches P_mech at steady state" do
      ...
      assert_in_delta gen_pelec, 0.8, 0.01, "P_elec should equal P_mech at steady state"
    end
  end
  ...
  describe "SMIB generator trip" do
    test "tripping generator (P_mech -> 0) causes deceleration" do
      ...
      # 2 seconds
      trajectory = Classical.simulate(state, 2000, 50)
    ...
    test "tripping generator causes rotor angle to decrease" do
      ...
      # 5 seconds
      trajectory = Classical.simulate(state, 5000, 100)
  ...
  describe "SMIB fault and stability" do
    test "small fault disturbance is stable (angle oscillates and damps)" do
      ...
      # fault on: P_mech jumps
      state = State.add_event(state, 0.05, 1, 1.5)
      # fault cleared: P_mech restored
      state = State.add_event(state, 0.15, 1, 0.8)

      # 3 seconds
      trajectory = Classical.simulate(state, 3000, 100)
    ...
    test "large sustained fault causes out-of-step" do
      ...
      # 5 seconds
      trajectory = Classical.simulate(state, 5000, 50)

      # At some point, the generator should slip a pole
      any_oos =
        Enum.any?(trajectory, fn point ->
  ...
  describe "three-machine system" do
    ...
    test "three generators maintain synchronism with balanced load" do
      ...
      state = %State{
        ...
        delta: [0.1, 0.0, -0.1],
        omega: [1.0, 1.0, 1.0],
        # Net power = 0 (no acceleration)
        p_mech: [0.3, 0.0, -0.3],
        e_prime: [1.0, 1.0, 1.0],
        # Middle machine is large (reference)
        h: [5.0, 100.0, 5.0],
        # Strong damping for convergence
        d: [5.0, 5.0, 5.0],
      ...
      # 20 seconds at 5ms
      trajectory = Classical.simulate(state, 4000, 100)
      ...
      for omega <- last.omega do
        assert_in_delta omega, 1.0, 0.05, "All generators should be near synchronous speed"
      end
    end
  end
