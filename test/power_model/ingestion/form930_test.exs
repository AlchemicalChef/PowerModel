defmodule PowerModel.Ingestion.EIA.Form930Test do
  ...
  test "prefers adjusted demand, falls back to raw when adjusted is blank" do
    ...
    assert demand_for("CISO", "2024-07-15T20:00:00Z") == 44_250.0
  end
  ...
  test "parses US-format UTC timestamps" do
    ...
    assert demand_for("ERCO", "2024-07-15T18:00:00Z") == 76_200.0
    assert demand_for("ERCO", "2024-07-15T21:00:00Z") == 81_900.0
  end
  ...
  test "creates BAs present in 930 but absent from the database" do
    ...
    assert sec != nil
    assert sec.name == "SEC"
    assert demand_for("SEC", "2024-07-15T18:00:00Z") == 1_250.0
  end
  ...
  test "re-ingestion is idempotent and refreshes values" do
    ...
    {:ok, ts, 0} = DateTime.from_iso8601("2024-07-15T18:00:00Z")
